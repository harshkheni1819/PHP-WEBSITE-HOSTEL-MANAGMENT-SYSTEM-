...
if(isset($_POST['btnins']))
{
    ...
    $dst="images/".$Photo;
    $q=mysqli_query($con,"insert into gallery_master values('','$gname','$Photo')");
    if($q)
    {
...
<form class="form-horizontal widget-box " role="form" method=post enctype="multipart/form-data">
	    ...
	    <label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Gname </label>
			<div class="col-sm-9">
				<input type="text" name="txtgname" placeholder="Enter Gname" class="col-xs-10 col-sm-5" required />
			</div>
	</div>
...
<?php
       ...
       while($row=mysqli_fetch_array($q))
       {
		...
		echo "<td colspan=2><a href=gallery_master_edit.php?x=$row[0]><image src=edit.png height=30 width=30></a>";
		echo "<a href=gallery_master_delete.php?x=$row[0]><image src=delete.png height=30 width=30></a>";
       echo"</tr>";
        }
        echo"</table>";
