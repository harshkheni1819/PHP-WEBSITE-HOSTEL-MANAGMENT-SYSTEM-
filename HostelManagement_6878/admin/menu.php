...
if(isset($_POST['btnins']))
{
    ...
    $q=mysqli_query($con,"insert into menu values('','$Day','$Breakfast','$Lunch','$Eveningsnacks','$Dinner')");
    if($q)
    {
...
<form class="form-horizontal widget-box " role="form" method=post>
    ...
    <div class="form-group">
	    <label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Day </label>
			<div class="col-sm-9">
				<input type="text" name="txtday" placeholder="Enter Day" class="col-xs-10 col-sm-5" required/>
			</div>
	</div>
    <div class="form-group">
	    <label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Breakfast </label>
			<div class="col-sm-9">
				<input type="text" name="txtbreakfast" placeholder="Enter Breakfast" class="col-xs-10 col-sm-5" required />
			</div>
	</div>
    <div class="form-group">
	    <label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Lunch</label>
			<div class="col-sm-9">
				<input type="text" name="txtlunch" placeholder="Enter Lunch" class="col-xs-10 col-sm-5" required/>
			</div>
	</div>
    <div class="form-group">
	    <label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Eveningsnacks </label>
			<div class="col-sm-9">
				<input type="text" name="txteveningsnacks" placeholder="Enter Eveningsnacks" class="col-xs-10 col-sm-5" required/>
			</div>
	</div>
    <div class="form-group">
	    <label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Dinner </label>
			<div class="col-sm-9">
				<input type="text" name="txtdinner" placeholder="Enter Dinner" class="col-xs-10 col-sm-5" required/>
			</div>
	</div>
...
<?php
       ...
       while($row=mysqli_fetch_array($q))
       {
         echo "<tr>";
         echo "<td>".$row['day'];
         echo "<td>".$row['breakfast'];
         echo "<td>".$row['lunch'];
         echo "<td>".$row['eveningsnacks'];
         echo "<td>".$row['dinner'];
		echo "<td colspan=2><a href=menu_edit.php?x=$row[0]><image src=edit.png height=30 width=30></a>";
		echo "<a href=menu_delete.php?x=$row[0]><image src=delete.png height=30 width=30></a>";
       echo"</tr>";
        }
        echo"</table>";
