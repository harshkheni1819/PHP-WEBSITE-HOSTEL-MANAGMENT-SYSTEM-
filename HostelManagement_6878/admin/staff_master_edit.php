...
<?php
$staffid=$_GET['x'];
$q=mysqli_query($con,"select * from staff_master where staffid=$staffid");
...
if (isset($_POST['btnup'])) {
    ...
    $dst="images/".$Photo;
    $q = mysqli_query($con, "update staff_master set stafftype='$Stafftype', photo='$Photo' where staffid=$staffid");
    if ($q)
     {
      move_uploaded_file($_FILES["txtphoto"]["tmp_name"],$dst);
        echo "<script>window.location.assign('staff_master.php')</script>";
    } else {
        echo "not updated..";
    }
}
...
<form class="form-horizontal widget-box " role="form" method=post enctype="multipart/form-data">
  ...
  <div class="form-group">
    ...
    <div class="col-sm-9">
    <input type="text" name="txtstafftype" placeholder="Enter Stafftype" class="col-xs-10 col-sm-5"  value="<?php echo $row['stafftype']; ?>"required/>
    </div>
</div>
<div class="form-group">
