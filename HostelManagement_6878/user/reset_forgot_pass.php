...
<head>
	<title>Aashray Stay Log IN</title>
	<meta charset="utf-8">
...
<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        $q = mysqli_query($con, "update user_master set password='$newpassword' where email='$email'");
       echo "update user_master set password='$newpassword' where email=$email";

        if ($q) {
            echo "<script>alert('Congratulations You have successfully changed your password');</script>";
            
            echo "<script>window.location.assign('r.php'); </script>";
        } else {
